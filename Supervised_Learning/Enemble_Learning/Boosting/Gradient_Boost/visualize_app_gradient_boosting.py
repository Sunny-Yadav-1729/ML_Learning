...
def Gradient_Boosting():
    ...
    st.sidebar.markdown("## Gradient_Boosting ")
    ...
    loss = st.sidebar.selectbox(
        label=f'Choose loss ',
        options=['squared_error', 'absolute_error', 'huber', 'quantile'] if typeOf == 'Regression'  else ['log_loss', 'deviance', 'exponential']
    )
    ...
    maxdepth = st.sidebar.number_input(
        label='Choose max_depth', 
        step=1, 
    ...
    if st.sidebar.button('Run Algorithm'):
        ...
        # clf.fit(X_train,y_train)
            ...
            if typeOf == problem_tpye[1]:
                bc = GradientBoostingClassifier(n_estimators=n_estimtor,loss=loss,learning_rate=learing_rate,max_depth=maxdepth)
                sc = cross_val_score(bc,X,y,cv=5,scoring='accuracy')
                st.sidebar.write(f" Gradient_Boosting  : {np.round(sc.mean(),2)}")
                m = bc
            else :
                br = GradientBoostingRegressor(n_estimators=n_estimtor,loss=loss,learning_rate=learing_rate,max_depth=maxdepth)
            ...
            if typeOf == 'Classification':
                ...
                plot_decision_region(axs[-1], bc, X, y, title=f"Gradient_Boosting ({typeOf})",typeof=typeOf)
            else :
                
                score = cross_val_score(br, X, y, cv=5, scoring='r2')
                st.sidebar.subheader(f"🧩 Gradient_Boosting R²: {np.round(score.mean(), 3)}")
                if X.shape[1] == 1:  # 1D regression
                    br.fit(X, y)
                    x_line = np.linspace(X.min(), X.max(), 100).reshape(-1, 1)
                    y_pred = br.predict(x_line)
                    axs[-1].scatter(X, y, color='blue', label='Data')
                    axs[-1].plot(x_line, y_pred, color='red', label='Gradient_Boosting Prediction')
                    axs[-1].set_title("Gradient_Boosting Regressor")
                else:  # 2D regression
