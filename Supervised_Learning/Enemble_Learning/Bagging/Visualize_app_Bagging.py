...
def Bagging():
    st.sidebar.header("Bagging Ensemble")
    ...
    n_feature = st.sidebar.number_input(
    ...
    if st.sidebar.button('Run Algorithm'):
        ...
        if not algorithms:
            ...
            if typeOf == problem_tpye[1]:
                bc = BaggingClassifier(m,n_estimators=n_estimtor,bootstrap=bootstrap,bootstrap_features=bootstrap_feature,max_samples=n_sample,max_features=n_feature)
                sc = cross_val_score(bc,X,y,cv=5,scoring='accuracy')
                st.sidebar.write(f" Voting  : {np.round(sc.mean(),2)}")
            else :
                br = BaggingRegressor(m,n_estimators=n_estimtor,bootstrap=bootstrap,bootstrap_features=bootstrap_feature,max_samples=n_sample,max_features=n_feature)
            ...
            if typeOf == 'Classification':
                ...
                plot_decision_region(axs[-1], bc, X, y, title=f"Bagging ({typeOf})",typeof=typeOf)
            else :
                
                score = cross_val_score(br, X, y, cv=5, scoring='r2')
                st.sidebar.subheader(f"🧩 Bagging R²: {np.round(score.mean(), 3)}")
                if X.shape[1] == 1:  # 1D regression
                    br.fit(X, y)
                    x_line = np.linspace(X.min(), X.max(), 100).reshape(-1, 1)
                    y_pred = br.predict(x_line)
                    axs[-1].scatter(X, y, color='blue', label='Data')
                    axs[-1].plot(x_line, y_pred, color='red', label='Bagging Prediction')
                    axs[-1].set_title("Bagging Regressor")
                else:  # 2D regression
