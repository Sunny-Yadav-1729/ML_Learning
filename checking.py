    ...
    if st.sidebar.button('Run Algorithm'):
        ...
        # clf.fit(X_train,y_train)
            ...
            if typeOf == problem_tpye[1]:
                bc = GradientBoostingClassifier(None,n_estimators=n_estimtor,algorithm=algo,learning_rate=learing_rate)
                sc = cross_val_score(bc,X,y,cv=5,scoring='accuracy')
                st.sidebar.write(f" Gradient_Boosting  : {np.round(sc.mean(),2)}")
                m = bc
            else :
                br = Gradient_BoostingRegressor(None,n_estimators=n_estimtor,loss=algo,learning_rate=learing_rate)
